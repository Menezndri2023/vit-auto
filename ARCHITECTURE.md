...
- **Nettoyage, migration des photos de profil, et test du coût de LECTURE (2026-09-10)** : (1) Les incohérences relevées la veille sont corrigées : l'annonce orpheline approuvée et disponible — donc réservable alors que personne ne pouvait l'honorer — est archivée, ainsi que 10 abonnements sans vendeur. `scripts/archiverAnnoncesOrphelines.mjs` reste disponible pour un futur rattrapage. (2) `scripts/migrerPhotosProfilVersImageKit.mjs` sort `User.profilePhoto` de la base : 1 927 Ko rapatriés sur ImageKit, **0 octet de photo restant en base**. La vitrine des partenaires passe de **23 s à 922 ms** mesurées sur la base réelle. Les pièces d'identité KYC (`identity.*`, 32 Mo) ne sont VOLONTAIREMENT pas migrées : ce sont des données personnelles sensibles servies aujourd'hui derrière une authentification, et les déposer sur un CDN sans URL signée serait une régression de confidentialité, pas une optimisation — cette migration demande d'abord de trancher le mode d'accès. (3) `tests/spotlightVolume.test.js` comble le trou que `publicRoutesVolume` ne pouvait pas voir : celui-ci mesure le POIDS DE LA RÉPONSE, or le défaut réel était du côté LECTURE — la réponse tenait en six partenaires tandis que la requête chargeait cent huit photos base64. Le nouveau fichier intercepte `Query.prototype.exec` pour observer les projections réellement envoyées à Mongo et vérifie : aucune photo de profil dans le vivier, photos lues uniquement pour les éléments retenus, une seule image par annonce (`$slice: 1`), et un nombre de requêtes indépendant du nombre de candidats. Piège rencontré en l'écrivant : un `schema.pre("find")` ajouté APRÈS la compilation du modèle n'est jamais appelé — les compteurs restaient vides et les tests passaient pour une mauvaise raison. (4) Vérification du catalogue : `/api/vehicles?limit=20` mesuré à 2,4 s depuis une connexion domestique se décompose en étapes de 230 à 820 ms, toutes dominées par la latence vers Atlas. Aucune pathologie serveur — la mesure initiale portait sur du réseau, pas sur du calcul.
- **Règlement en espèces étendu, garde-fou de prix, et rattrapage des réservations anciennes (2026-09-10)** : (1) **Espèces.** La règle « tout se règle sur place » ne couvrait que la LOCATION, en dur dans le contrôleur. Les activités de loisir y échappaient : leur écran de réservation proposait Orange Money par défaut et la carte bancaire, et le serveur enregistrait un paiement « pending » sans qu'aucun prestataire ne vérifie quoi que ce soit — un client pouvait croire avoir payé sa sortie en quad. Le périmètre est désormais nommé dans `constants/paiement.js` (location, activité, chauffeur ; l'essai est gratuit et le leasing engage un financement), appliqué côté serveur ET miroir côté interface, avec la mention explicite au client. Un test vérifie que les deux listes coïncident : un écran qui propose un moyen que le serveur refuse produit un échec incompréhensible au moment de valider. (2) **Plausibilité des montants** (`constants/plausibilitePrix.js`). Une Changan CS55 était publiée à 60 600 MAD/jour, soit 6 110 USD, quand les 335 locations marocaines publiées tiennent entre 23 et 151 USD (médiane 30) ; elle était en ligne et réservable depuis le 3 août, approuvée, et rien ne la signalait. Plafonds d'ABSURDITÉ et non de marché — 2 000 USD/jour laisse passer une voiture de sport — appliqués à la création et à la modification. Nuance importante trouvée par la suite : la garde ne valide QUE les montants que la requête touche, sinon une annonce portant un tarif hérité ne pouvait plus recevoir la moindre correction de description, ce qui enfermait le partenaire dans son erreur. L'annonce a été remise en `pending` avec son motif, et une relance calculée depuis la base (`scripts/relancerAnnonceACorriger.mjs`, simulation par défaut) a été envoyée au partenaire avec la médiane réelle de son pays. (3) **Rattrapage** (`scripts/reparerReservationsAnciennes.mjs`) : trois réservations de juin 2026 n'avaient pas de `reference` et apparaissaient sous leur identifiant brut sur les contrats et factures — références reconstruites avec leur année d'origine. Trois enregistrements de paiement ne référencent aucune réservation (un « cash » de 2 000 000, deux « card » alors qu'aucune passerelle n'a jamais existé) : listés, jamais supprimés automatiquement — un enregistrement financier ne se détruit pas sur la foi d'un script. (4) **Audit photos** : 346 véhicules et 10 activités publiés, toutes premières images sur ImageKit, zéro base64, zéro hotlink, et les 356 URL vérifiées une par une répondent. Aucune annonce publiée sans titre ni sans prix. L'annonce orpheline sans réservation liée a été supprimée ; les deux autres, portant des réservations, restent archivées — les supprimer laisserait des références pendantes dans l'historique.
- **Un prix douteux ne remonte plus en première page (2026-09-10)** : le moteur de validation signalait déjà « Tarif journalier très élevé — vérifiez le montant » sur la Changan à 6 110 USD/jour (score 92). L'avertissement n'est pas bloquant, personne ne l'a lu à l'approbation, et l'annonce a passé cinq semaines en vitrine d'accueil. Ces avertissements portent désormais un MARQUEUR stable (`MARQUEUR_MONTANT_DOUTEUX = "[MONTANT]"`, `constants/plausibilitePrix.js`) que le moteur de mise en avant exclut de toutes ses vitrines. Un marqueur et non une recherche de tournure : reformuler le message suffirait sinon à rouvrir la porte sans que personne s'en aperçoive — et les crochets, qui sont une classe de caractères en expression régulière, sont échappés, faute de quoi le filtre ne correspondrait à rien et la garde serait silencieusement inopérante. L'annonce reste au CATALOGUE, où le visiteur arrive en cherchant ; elle ne revient plus en première page, où un prix douteux coûte plus qu'il ne rapporte. Le filtre vise le marqueur et non « tout avertissement » : une annonce dont la seule faute est une description courte garde sa place, et les deux régressions — filtre trop étroit, filtre trop large — font chacune tomber un test. La seule annonce concernée en production a été rattrapée.
- **Page blanche pour un pays sans annonce — repli mondial (2026-09-11)** : incident de production signalé comme « aucune image n'est visible ». Les images n'étaient pas en cause : 26 URL transformées, testées telles que le navigateur les demande, répondent toutes en 200, comme les 106 morceaux de page et l'API. Le catalogue était VIDE. Les 345 annonces publiées sont au Maroc (335) et en France (10) ; la dernière annonce ivoirienne était l'annonce orpheline sans propriétaire, retirée la veille — retrait justifié, mais qui a vidé d'un coup toute la page pour un visiteur en Côte d'Ivoire. Point aggravant : le catalogue ne passe AUCUN pays à l'API, il charge tout et filtre côté client (`paysOk` dans Catalogue.jsx) ; le filtre serveur n'était donc même pas le chemin emprunté. Correctif à deux endroits, avec la même règle : quand le pays du visiteur ne laisse passer aucune annonce, la restriction de pays est levée et l'international s'affiche — côté client sur le jeu de données de la section affichée (il peut y avoir des véhicules près de lui mais aucune activité de loisir), côté serveur sur `getVehicles` pour les appels qui passent un pays. Deux garde-fous : une RECHERCHE explicite ne déclenche jamais le repli — chercher « Ferrari » et recevoir des Dacia serait pire que zéro résultat — et le repli lève la restriction de pays, jamais les critères choisis par le visiteur. L'interface l'ANNONCE, sans quoi un visiteur ivoirien à qui l'on montre des voitures marocaines croit à une erreur. Vérifié sur la base réelle : catalogue 0 → 345 annonces, vedettes 0 → 8, carrousel 0 → 6, toutes avec image ; recherche « Ferrari » toujours à 0. Erreur de méthode à retenir, commise DEUX fois : avoir signalé un 404 sur une URL ImageKit tronquée par mon propre affichage — les deux avatars migrés répondent en 200. Comparer les URL complètes, jamais ce qu'affiche le terminal.
- **Balayage de la production au navigateur — CSP, mobile, app (2026-09-11)** : après le correctif du service worker, 88 chargements de pages publiques audités dans un vrai navigateur, sur quatre écrans (390, 360, 768, 1280 px). Résultat : **0 image cassée, 0 erreur JavaScript grave, 0 débordement horizontal, 0 page vide**. Trois constats. (1) **CSP incomplète** : `accounts.google.com` était autorisé pour `script-src`, `frame-src` et `connect-src`, mais PAS pour `style-src` — la feuille `accounts.google.com/gsi/style` de Google Sign-In était bloquée sur `/login`, `/register` et `/importer-apply`, c'est-à-dire les trois pages où l'on se connecte. Une CSP qui autorise un tiers à moitié est pire qu'un refus franc : le bouton s'affiche, mal, et rien ne l'explique. Corrigé, avec un test qui vérifie les quatre directives ensemble. (2) **Fausse alerte de ma méthode** : `/register` et `/importer-apply` semblaient ne jamais finir de charger. Vérification faite, la page est complète (5 384 caractères, 0 requête ouverte après 20 s) — c'est le websocket Socket.io qui empêche `networkidle` d'être atteint. Le critère d'attente était en cause, pas la page. (3) **Cibles tactiles** : 375 éléments sous 40 px sur le catalogue mobile, mais le décompte brut trompait — il s'agit du même bouton « favori » répété une fois par annonce (34 px) et des flèches du carrousel (38 px). Tous deux portés à 44 px sur mobile ; ils sont posés en absolu par-dessus la photo, l'agrandissement ne décale rien. Restent les liens de pied de page à 17 px de haut, signalés et non touchés : les agrandir reflowerait le pied de page, ce qui n'est pas un correctif à glisser dans une passe d'incident. **Application mobile** : `capacitor.config.json` pointe `server.url` sur `https://vit-auto.com` — l'app affiche le site en direct, sans paquet séparé. Elle héritait donc de la panne, et hérite du correctif ; aucune reconstruction native n'est nécessaire. Vérifié au passage qu'aucun code ne s'appuie sur `navigator.userAgent` (l'app le remplace par `VIT-AUTO-iOS`/`VIT-AUTO-Android`), la détection passant par `Capacitor.isNativePlatform()` — la mise en page ne dépend donc que des media queries, que ce balayage couvre.
- **Affichage mobile : le hero débordait de 790 px, et 15 titres étaient invisibles (2026-09-11)** : signalé par une capture d'écran de l'exploitant montrant le contenu coupé à droite sur iPhone. Mon balayage précédent n'avait rien vu, et c'est instructif : il mesurait `scrollWidth - clientWidth`, or le hero porte `overflow: hidden` — le contenu trop large n'était pas DÉFILABLE mais ROGNÉ, donc `scrollWidth` restait égal à la largeur visible. La bonne mesure compare la position réelle de chaque élément (`getBoundingClientRect().right`) à la largeur du viewport, en ignorant ce qui vit dans une bande volontairement défilante et la décoration sans texte. Quatre défauts corrigés. (1) **Hero à 1180 px sur un écran de 390** : une colonne de grille a un `min-width: auto` implicite, et la bande des pays d'origine — `flex` sans retour à la ligne — élargissait la colonne jusqu'à sa largeur de contenu ; `minmax(0, …)` sur TOUTES les déclarations, y compris les surcharges tablette où le test a montré que le défaut serait revenu, plus `min-width: 0` sur les rangées pleine largeur et `overflow-x: auto` sur la bande des pays. (2) **15 titres illisibles** : `h1..h6 { color: <valeur fixe> }` bat systématiquement l'héritage, si bien qu'un titre sans classe placé dans une section sombre recevait le bleu nuit de la charte sur fond bleu nuit — c'était le cas du titre de la page Tarifs et de ceux des six pages légales. Passé à `color: inherit`. Estimation initiale erronée et corrigée par la mesure : j'avais prédit 96 titres affectés et un risque sur le logo, en supposant que chaque titre prendrait la couleur de son parent ; en injectant réellement la règle dans le navigateur, page par page, le résultat est 15 corrections et aucun effet de bord — les titres portant une classe gardent la leur, une règle de classe l'emportant sur une règle d'élément. (3) **Flèches du carrousel par-dessus le titre de l'annonce** : masquées sous 640 px, une carte de cette largeur n'ayant pas la place pour des commandes latérales. (4) **Indicateurs déformés en ellipses de 7 × 44 px** par la règle globale `button { min-height: 44px }` : la règle est conservée — elle protège les vraies commandes — et la zone tactile est rendue par un pseudo-élément, si bien que la pastille reste un point de 7 px avec une cible de 44. **Outillage** : `vite preview` n'avait aucun relais d'API, le paquet construit ne pouvait donc pas être exercé localement ; un relais vers l'API de production est ajouté, avec `npm run verify:local` (navigateur réel, trois écrans, sortie en échec si anomalie) et `npm run verify:prod`. Quatorze tests de garde couvrent désormais ce qu'aucune suite ne regardait : service worker, CSP, et ces règles CSS — chacun vérifié à l'envers. Deux d'entre eux ont d'abord échoué sur un commentaire citant le code fautif : toute analyse de source retire les commentaires d'abord.
- **Publication App Store — diagnostic et chaîne de publication sans Xcode local (2026-09-11)** : l'app « ne se publiait pas » alors que l'Apple Developer Program était payé depuis le 28 juillet. Constat : rien n'avait jamais été envoyé à Apple — le compte est resté « en attente » (Apple encaisse puis vérifie l'identité ; délai officiel 24–48 h, forums 2026 : 2 à 7 semaines), donc pas de Team ID, pas de certificat, pas d'archive, pas de fiche App Store Connect ; la machine n'a que les Command Line Tools ; et la CI mobile échouait **depuis sa création** pour deux raisons masquées l'une par l'autre : Node 20 alors que Capacitor 8 exige Node ≥ 22 (`cap sync` refusait de démarrer sur les deux plateformes), puis `pod install` + `App.xcworkspace` alors que le projet est en **Swift Package Manager** (`ios/App/CapApp-SPM`, aucun Podfile). Corrections : Node 22 et compilation du `.xcodeproj` sans CocoaPods dans `mobile.yml` ; nouveau `ios-release.yml` (déclenchement manuel) qui archive, signe et envoie à App Store Connect depuis le runner macOS via **cloud signing** — une clé d'API App Store Connect (rôle Admin) suffit, Xcode crée lui-même certificat, App ID et profil (`-allowProvisioningUpdates` + `-authenticationKey*`), et le workflow s'arrête à la première étape en listant les secrets manquants tant que le compte n'est pas actif. Projet Xcode complété pour la review : scheme partagé `App.xcscheme` (sans lui, `xcodebuild -scheme App` n'a rien à résoudre sur un checkout propre), `App.entitlements` avec `aps-environment` (le plugin push échouait sinon à l'enregistrement), `PrivacyInfo.xcprivacy` déclarant les données réellement collectées (à reporter tel quel dans les étiquettes App Privacy, une incohérence vaut rejet), `ITSAppUsesNonExemptEncryption=false` (supprime la question de conformité export à chaque envoi). Le manuel `docs/manuels/publication-app-store.md` couvre le déblocage du compte (quoi préparer pour le support Apple), les 4 secrets, la fiche, le compte de démonstration exigé par la règle 2.1 et les notes de review anticipant la règle 4.2 (« site web emballé » — l'app charge `https://vit-auto.com` en direct). Le pbxproj a été validé par `plutil -lint` et par un contrôle d'intégrité des références ; la compilation réelle n'est vérifiable que par la CI.
- **Capacitor 8.3.1 → 8.5.2 et deux outils d'icônes retirés (2026-09-13)** : `npm audit` racine signalait `tar` CRITIQUE (écrasement de fichiers à l'extraction). Après la montée de `@capacitor/{cli,core,ios,android}` à 8.5.2 (tar 7.5.22, corrigé), la critique **persistait** — elle venait de `@capacitor/assets@3.0.5`, qui embarque son propre `@capacitor/cli@5.7.8` et donc un `tar@6.2.1`. `@capacitor/assets` et `sharp` (CVE libvips sans correctif) servaient uniquement à la génération, déjà faite et committée, des icônes et splash de marque (`resources/`, `ios/App/App/Assets.xcassets`, `android/app/src/main/res`) ; aucun script du dépôt ne les importe. Retirés des devDependencies : audit racine 7 → 3 vulnérabilités, **0 critique, 0 haute**, les 3 modérées restantes (`xcode`, `uuid`) étant internes au CLI Capacitor sans correctif amont. Pour regénérer les icônes un jour : `npx @capacitor/assets generate --iconBackgroundColor '#0f1b3f'` sans l'installer. `cap sync` a réécrit `CapApp-SPM/Package.swift` sur `capacitor-swift-pm` exact 8.5.2 ; tests front 74/74, build OK ; la compilation native reste vérifiée par la CI mobile au push.
- **Accès partenaires par secteur, cumul et quotas par plan (2026-09-14)** : l'activité choisie à l'inscription n'était relue nulle part — un centre de plongée pouvait publier une voiture par requête directe. Trois axes désormais séparés et documentés (`docs/manuels/acces-partenaires.md`) : le **secteur** (identité validée par l'administration : ce que le compte publie et voit, son taux de commission), le **type d'entité** (documents exigés) et le **plan** (outils, visibilité, et deux nouveaux leviers). (1) `server/utils/perimetre.js` — `refusDePerimetre(user, secteur, action)` est le pendant serveur du masquage du dashboard (`couvreSecteur` côté front, extension du `estUniquementLoisirs` de la veille) ; posé sur les cinq portes de publication (véhicule location→loueur / vente→vendeur, activité, profil chauffeur, annonce export et conversion en export). Un compte historique sans secteur n'est pas bloqué. (2) `server/services/quotaAnnonces.js` — quota d'annonces ACTIVES par secteur selon le plan (`PLAN_QUOTA_ANNONCES` 5/15/60/illimité), trois exemptions dans l'ordre : immunité de lancement jusqu'au 2027-09-10 (`FIN_IMMUNITE_QUOTAS`, même mécanisme daté que la vitrine partenaires gratuite), Partenaire Fondateur en cours (dossier SIGNÉ, `lockedAt` posé — un dossier sans date n'exempte pas, même règle que la commission), plan sans limite ; un membre d'équipe consomme le quota du titulaire ; le quota ne dépublie jamais, il bloque au-delà à la création (`QUOTA_ANNONCES`). Compteurs enregistrables (`enregistrerCompteur`) pour les secteurs dont le modèle vit ailleurs — « pièces détachées », ajouté à la taxonomie ce jour pour la session voisine, compte zéro tant que son module ne s'enregistre pas. (3) **Cumul de secteurs = avantage de plan** (`PLAN_SECTEURS` 1/1/2/tous) mais jamais un droit : le secteur s'ajoute par une demande (`PartnerSectorRequest`, `/api/partner-sectors`) que l'administration accorde ou refuse, avec le contexte du compte ; la limite est vérifiée à la demande ET à l'approbation (le plan a pu changer entre les deux). Un loueur qui propose ses véhicules avec chauffeur utilise l'option `withDriver` de son annonce — ce n'est pas un second métier. (4) Page Tarifs : les paliers portaient un nom de métier (« Exportateur ») ou de type d'entité (« pour les particuliers »), en contradiction avec les axes ci-dessus — libellés Essentiel / Business / Premium, identifiants en base inchangés ; onglets par secteur (`OUTILS_PAR_SECTEUR`, uniquement de l'existant — un test verrouille l'absence de « bientôt ») ; la promesse « commission réduite », fausse depuis la grille du 2026-09-09, retirée ici et sur la page Import/Export. Limite assumée : les outils par secteur sont PRÉSENTÉS par palier, plusieurs restent accessibles à tous (import de flotte, PMS, showroom) — un compte gratuit reçoit plus que promis, jamais moins ; leur verrouillage (`FEATURE_MIN_PLAN` + `exigeFonctionnalite`) est la prochaine étape.
- **Outils par secteur verrouillés par plan (2026-09-14, suite)** : la limite assumée du matin est levée — les six outils vendus par palier sur la page Tarifs (tarifs saisonniers, promotions : Essentiel ; journal du véhicule, import de flotte, showroom, CRM leads/devis : Business) sont désormais refusés côté serveur par `exigeOutil` (`services/planAccess.js`), posé sur les ÉCRITURES seulement (routes vehicles, vehicleImport, pms). Il diffère d'`exigeFonctionnalite` par trois passe-droits qu'un OUTIL exige et qu'un avantage n'a pas : l'administrateur (il agit sur les annonces de n'importe quel partenaire, sans abonnement), l'immunité de lancement jusqu'à `FIN_IMMUNITE_QUOTAS` (activer la règle aujourd'hui retirerait l'import de flotte ou le showroom à des comptes qui s'en servent — quotas et verrous d'outils prennent effet le même jour, une seule constante à avancer), et le Partenaire Fondateur en cours. La définition du fondateur actif (dossier signé + douze mois) sort de quotaAnnonces vers `services/fondateur.js`, partagée sans cycle d'import. L'horloge est injectable (`maintenant`) : la règle dépend d'une date, les tests la passent explicitement — « aujourd'hui » est immunisé par construction.
- **Audit d'affichage multi-formats et cibles tactiles (2026-09-15)** : nouveau `scripts/auditAffichage.mjs`, lecture seule, au-delà de la garde (qui attrape plantages, images cassées, débordements, police) : défilement horizontal, cibles tactiles < 44 px sur écran tactile, textes coupés, contenu injoignable sous la barre du bas, texte < 11 px (minimum Apple), chevauchement d'éléments fixes — sur iPhone SE, iPhone 6,7", iPad portrait et bureau, pages publiques, client et partenaire (comptes semés par `apiLocale.mjs`). Premier passage : 102 pages/écrans en défaut ; après corrections : 0. Trois défauts structurels expliquaient l'essentiel. (1) Les règles tactiles globales d'`index.css` étaient conditionnées à `max-width: 640px` : un **iPad** (1024 px, un doigt) n'en recevait aucune — 180 boutons, champs et listes sous 44 px là où le téléphone était propre. Nouveau bloc `(hover: none) and (pointer: coarse)` : la condition juste est le pointeur ; hauteur IMPOSÉE par `min-height`, jamais additionnée par du padding ; cases, radios et curseurs exclus. Les règles de composant plus spécifiques (`.actions button { min-height: 40px }` des cartes) sont alignées une à une. (2) La réserve pour la BottomNav (`mainWithBottomNav`) était posée sur `<main>`, or le pied de page vient APRÈS `<main>` : ses derniers liens (Partenaires, Mentions légales, FAQ) restaient sous la barre en bas de défilement, 204 occurrences sur 34 pages → `pageWithBottomNav` sur la page entière, 58 px (la barre mesure 57). (3) Le guide de bienvenue, à 18 px du bas, recouvrait la barre elle-même sur téléphone (bas de carte à 834 px sur 844, barre à 787) → relevé de la même réserve sous 900 px. Liens stylés en bouton (chips du hero, villes du catalogue, actions du PMS, CTA d'état vide, lien retour, appel du partenaire, liens en style inline du tableau de bord) portés à 44 px de haut par classe ; police des badges, étiquettes et libellés de champs relevée de 9,9–10,9 px à 11,2 px. Effet de bord corrigé au passage : « Voir les plans → » en `nowrap` écrasait le texte de la bannière du plan sur téléphone — la bannière passe en `flex-wrap`, le lien dessous. Pièges de mesure documentés dans le script : `scroll-behavior: smooth` rend `scrollTo` asynchrone (mesurer juste après lit la page en haut — 34 faux positifs), le splash se désactive par `sessionStorage.vit_splash_shown`, les libellés de la barre du bas à 10,6 px sont conformes (iOS utilise 10 pt pour sa tab bar), les puces du carrousel portent leur zone tactile par `::after`, le bouton Google Sign-In est rendu par Google (40 px). `scripts/capturesAppStore.mjs` produit les captures App Store aux quatre formats acceptés depuis le site de production, compte de démonstration connecté ; `server/scripts/creerCompteReviewApple.mjs` crée ce compte (client vérifié, score KYC 96 — sans score, la réservation affichait « INSUFFISANT — 0/100 » à un compte pourtant vérifié).
- **Parcours de service bout en bout, client et partenaire (2026-09-15)** : `scripts/parcoursServices.mjs`, ajouté à la garde après les parcours critiques, déroule sur la pile locale ce que la garde ne couvrait pas : **location** (réservation depuis le site avec documents joints à la dernière étape, prolongation, machine à états de la remise `in_progress → client_arrived → transaction_concluded → waiting_client_validation → completed`, avis croisés), **loisirs** (réservation depuis le site, prix par personne, séance, clôture), **import/export** (secteur demandé et accordé, profil importateur vérifié, annonce approuvée et publique, demande client, réservation, offre finale, séquestre déclaré puis vérifié, documents joints, expédition, suivi, livraison, fonds libérés, tableau de bord), **partenaire** (assistant de publication en sept étapes piloté par un remplisseur générique, modération, fiche publique, cumul de secteurs refusé au plan gratuit, PMS lead → relance → devis → showroom publié et rendu, aperçu d'import de flotte), **compte client** (inscription depuis le site, favoris, panier, profil, notifications, chat depuis une réservation et chat support, fidélité, KYC). Chaque scénario est rejouable sur une base qui persiste (dates décalées aléatoirement, modèle unique par exécution, profils déjà vérifiés conservés). Trois défauts réels trouvés et corrigés : (1) le **slug du showroom** naissait dans un hook `pre("save")` que `findOneAndUpdate` — seul chemin de création et de publication — n'exécute jamais : un showroom publié n'avait pas d'adresse publique, et la vitrine aurait pointé sur `/showroom/undefined` ; calculé dans le contrôleur, unique entre partenaires, suivant le nom avant publication et stable après ; (2) l'**assistant de publication** proposait « Location », « Vente », « Chauffeur » à un compte qui ne couvre pas ces secteurs et n'échouait qu'à l'étape 7, par un 403 serveur (`SECTEUR_REQUIS`) peu visible — les types hors secteur sont désormais grisés à l'étape 2 avec renvoi vers « Mes secteurs », et le brouillon d'un type non couvert est refusé ; (3) une valeur hors énumération à la soumission du profil importateur renvoyait un **500 muet** — 400 nommant le champ. Défaut de la pile locale corrigé au passage : aucune devise ni aucun pays actif n'étaient semés (`ExchangeRate`, `CountryConfig`), si bien que toute devise d'affichage choisie à la publication était refusée et que la conversion des prix n'était jamais exercée par la garde — les données de référence sortent de la migration de production vers `server/config/defaultCurrencies.js`, partagées par la migration et par `apiLocale.mjs`. Hors production, l'inscription se termine sans code e-mail (dev sans SMTP, `isDevNoSmtp`) : le scénario accepte les deux issues.
- **Page de secours sur la réservation d'un chauffeur — et pourquoi rien ne l'avait vue (2026-09-15)** : en production, tout VISITEUR (et tout client non vérifié) ouvrant `/driver-booking/:id` tombait sur l'écran « Une erreur s'est produite » : `ReferenceError: Cannot access 'D' before initialization`. Cause : dans `DriverBooking.jsx`, `const identitySatisfied = user?.kycStatus === "VERIFIE" || !!idFrontImage` était écrit vingt lignes AU-DESSUS du `useState` qui déclare `idFrontImage`. Le `||` court-circuitait pour un client vérifié — la variable n'était jamais lue — et lisait la constante dans sa zone morte pour tous les autres. La garde, les tests de rendu et le premier audit visitaient cette page connectés en admin ou avec le compte de démonstration VÉRIFIÉ : précisément le cas qui masquait le défaut. Même famille que la panne totale de l'admin du 2026-09-08 (tableau de dépendances). Trois garde-fous, pour que cette famille ne repasse plus : (1) **règle ESLint locale** `vit/lecture-avant-declaration` (`eslint-rules/`) — ne retient qu'une référence évaluée dans le MÊME corps de fonction que la déclaration et située avant elle ; `no-use-before-define` était inutilisable (130 faux positifs sur des usages différés) ; vérifiée à l'envers sur la version fautive ; exécutée SEULE et bloquante en tête de la garde (`scripts/verifierTDZ.mjs`), car le lint complet porte des erreurs historiques non bloquantes et la CI le lance en `|| true` ; (2) **tests de rendu** `screens.chauffeur.render.test.jsx` : pages chauffeur hors connexion et avec un client non vérifié, échouent sur la version fautive ; (3) **parcours « visiteur »** dans `parcoursServices.mjs` : toutes les pages dynamiques (véhicule, réservation, chauffeur, embauche, activité, annonce export, showroom, profil partenaire, pages de ville et de marque, catalogues) ouvertes hors connexion puis avec un client tout juste inscrit, échec à la moindre page de secours. Ce parcours a révélé un second défaut : le **cache catalogue** (30 s, par processus) n'était invalidé nulle part — `cacheClear()` n'avait aucun appelant — si bien qu'une annonce vendue restait listée jusqu'à trente secondes et que son ouverture répondait « Annonce introuvable ». Invalidation désormais portée par les MODÈLES (`post` sur save/update/delete des schémas Vehicle, ImportExportListing, Activity, Driver) : tout chemin d'écriture, y compris `findByIdAndUpdate` depuis les transactions, vide le cache ; test `catalogCacheInvalidation.test.js`. `SparePart` (session voisine) reste à aligner.
- **App iOS : bande vide sous la barre d'état (2026-09-15)** : dans l'app native (capture TestFlight), une bande claire de ~50 pt séparait la barre d'état de l'en-tête. La zone de sécurité de l'encoche était réservée TROIS fois : par le conteneur natif (`capacitor.config.json` `contentInset: "automatic"` — zone bleu nuit, couleur de fond du WebView), par `body { padding-top: var(--safe-top) }` (la bande claire, deux fois : règle de base et bloc `display-mode: standalone`), et par la barre de navigation — qui, seule, DEVRAIT l'absorber (sombre sous une barre d'état claire, l'ensemble ne fait qu'un) mais perdait son `padding-top` sous 900 px, écrasé par un `padding: 0 16px` en raccourci. Reproduit en local en forçant `--safe-top: 54px` (toutes les règles passent par cette variable), corrigé et revérifié de la même façon : une seule réserve, dans la barre, qui grandit d'autant (`--navbar-total` = barre + encoche, utilisée par tout ce qui se cale sous l'en-tête : panneau loisirs, panneau notifications, hauteurs minimales des pages) ; `contentInset: "never"` côté natif (nécessite un nouveau build iOS) ; toasts d'administration et de certification décalés de l'encoche ; toasts mobiles relevés au-dessus de la BottomNav. Deux tests de garde (`css.test.js`) verrouillent le schéma — vérifiés à l'envers.
- **Documents adaptés au service et à l'entité + en-tête téléphone (2026-09-15)** : règle de l'exploitant — les pièces demandées dépendent de QUI propose le service et de ce que le client engage, pas d'un KYC générique. Côté client : une mission chauffeur ne demande plus de pièce d'identité (`eligibilityEngine` ne compte plus `chauffeur` parmi les réservations qui exigent l'identité — seules location et essai, où le client conduit, la conservent ; la section « Pièce d'identité » de `DriverBooking.jsx` est retirée) ; une activité de loisirs ne demande plus de numéro de passeport ; un achat de pièce reste sans document, l'importation seule en exige. Côté partenaire : un chauffeur `particulier` publie son profil avec sa pièce d'identité et son permis ATTACHÉS AU PROFIL (`DRIVER_DOCS_REQUIRED` toujours vérifié) sans passer d'abord par la certification (`createDriver` ne lui applique plus `refusDePublication`), le bandeau d'orientation vers `/kyc` de `VendorSubmit` devient une information ; une entreprise reste tenue à la vérification d'entreprise. Matrice complète dans `docs/manuels/acces-partenaires.md`. Tests : `eligibilityEngine.test.js` (mission chauffeur sans identité), `driver.create.test.js` (particulier sans KYC : avec pièces → en attente, sans pièces → 400). Barre de navigation sur téléphone : mesurée à 320/375/430 px, trois commandes de trois styles et, à 320 px connecté, la rangée de droite passait à la ligne (barre à deux étages) — en-tête ramené à 56 px (`--navbar-h` sous 640 px, tout ce qui se cale sous l'en-tête suit la variable), logo réduit par sa prop (`useIsMobile`), langue/cloche/burger au même gabarit de 44 px sans cadre, aucun retour à la ligne possible, menu ouvert en superposition sous la barre au lieu de la déformer.
- **Réponse à la demande d'information Apple (2.1) — suppression réelle du compte, blocage dans le chat, compte partenaire de démo (2026-09-17)** : premier verdict de l'App Review sur la 1.0 (build 6) = « Information Needed » (vidéo d'écran, description, identifiants par type de compte, services tiers, différences régionales, secteur réglementé). Trois manques réels révélés par la liste d'Apple, corrigés : (1) **« Supprimer mon compte » ne supprimait rien** — `deactivateMyAccount` posait `isActive:false` « réversible via le support », ce que la règle 5.1.1 (v) juge explicitement insuffisant ; désormais suppression pure quand aucune réservation ne cite le compte (annonces non réservées supprimées, réservées archivées, loisirs/pièces/export archivés), sinon anonymisation définitive (identité, pièces, photo, téléphone, adresse effacés, e-mail remplacé donc libéré, mot de passe aléatoire, jetons révoqués, `deletedAt`) pour que les réservations et factures des autres parties restent lisibles ; tests `selfDeactivation.test.js` réécrits (suppression, anonymisation, réinscription avec la même adresse). (2) **Aucun blocage d'utilisateur** et aucun signalement dans le chat (règle 1.2 : signaler ET bloquer) — `User.blockedUsers`, `POST/DELETE /api/users/:id/block`, refus 403 `BLOCKED` dans les deux sens à l'envoi et à l'ouverture d'une conversation client ↔ partenaire (jamais sur le support), `blockedByMe` renvoyé par `GET /api/chats`, boutons Signaler (ReportButton, cible `user`) et Bloquer/Débloquer dans l'en-tête de la conversation, bandeau remplaçant la saisie ; tests dans `chat.test.js`. (3) **Un seul compte de démonstration** alors qu'Apple exige des identifiants pour chaque type de compte — `creerCompteReviewApple.mjs --partenaire` crée `review-partner@vit-auto.com` (entreprise loueur/vendeur/chauffeur, fondateur, identité vérifiée, publication autorisée, compte de test donc invisible des vitrines). Réponse en six points, scénario de vidéo et notes : `docs/manuels/fiche-app-store.md`. Point d'attention formulé pour Apple : aucun paiement ni achat intégré dans l'app (réservations réglées chez le partenaire, abonnements partenaires contractés hors app, page /plans informative) — ne pas brancher un paiement en ligne de fonctionnalités numériques sans passer par l'achat intégré Apple.
- **Téléphone : messagerie sous l'encoche, appuis « qui ne passent pas », démarrage plus rapide (2026-09-18)** : l'exploitant signalait qu'une fois dans une conversation il était « impossible de ressortir ou de bloquer », que des boutons exigeaient plusieurs appuis, et que l'app était lente. Audit d'affichage étendu (`scripts/auditAffichage.mjs`) : encoche simulée (`--safe-top: 47px`, comme l'app native qui place la page SOUS la barre d'état), détection des commandes fixes dont le centre tombe dans cette zone, et « appui intercepté » (`elementFromPoint` au centre de chaque commande visible ≠ la commande), plus des états à ouvrir (messagerie liste/conversation, dialogue de suppression, menu, notifications) ; `<details>` fermés exclus (Chromium leur garde des boîtes). Constats et corrections : (1) la fenêtre de chat plein écran partait de `top: 0` sans réserve d'encoche — ←, Signaler, Bloquer et ✕ sous l'heure et la batterie ; en-tête qui absorbe `--safe-top`, ← et ✕ à 44 px, Signaler/Bloquer regroupés dans un menu « ⋯ » (deux boutons en ligne débordaient à 375 px) ; (2) tiroir du PMS partenaire sous la barre de navigation (z 100 < 500) et sous l'encoche ; (3) commandes qui n'apparaissaient qu'au survol (`opacity: 0` → `:hover`) : bouton « supprimer » des notifications, flèches du carrousel photo, voile des pièces, croix des photos — sur iOS le premier appui ne fait que « survoler », d'où le second appui ; toujours visibles sous `(hover: none)` ; (4) rangée des pays d'origine de l'accueil posée SUR la carte de recherche à 430 px (règle de base `grid-row: 3` écrite après le bloc portrait) ; pastilles d'état qui débordaient ; cibles < 44 px (Toast, WelcomeGuide, Celebration, notifications, « Voir les plans »). Vitesse : le splash React durait 3,5 s fixes — désormais ≥ 0,9 s et effacé dès que la session est validée (≤ 4 s) pendant que l'app se rend dessous ; une session connue (jeton + profil en local) démarre immédiatement et se valide en arrière-plan (avant : un aller-retour `/api/auth/me` bloquant, jusqu'à 16 s sur réseau capricieux) ; catalogue affiché depuis un cache local (localStorage, 15 min) puis rafraîchi, pages 2..n chargées en parallèle au lieu d'en séquence ; images des listes (vitrine, catalogue loisirs/chauffeurs/pièces/export, favoris, héros) passées par `optimizedImageUrl` (largeur 640/1200) au lieu de la pleine résolution ; cache serveur du catalogue porté de 30 s à 10 min (les hooks des modèles l'invalident à chaque écriture depuis le 15/09). Mesuré à 390 px avec 600 ms de latence API simulée : premier écran 4,1 s → 2,4 s (visiteur), 3,7 s → 1,8 s (client connu). Hors code : l'API Render répond en 0,4–0,7 s minimum depuis le Maroc/la Côte d'Ivoire (région US) — rapprocher le service (Francfort) est le levier restant.
- **App iOS figée sur le logo quand le site ne répond pas (2026-09-19)** : « je n'arrive pas à ouvrir l'app » — ni l'app ni Safari sur le téléphone ne joignaient vit-auto.com, alors que le site répondait depuis l'extérieur (déploiement Vercel READY, 3,8 s depuis un serveur tiers) : depuis le poste, `curl` mettait 75 s (une des deux adresses anycast de Vercel injoignable depuis l'opérateur, bascule sur la seconde après expiration TCP). L'app chargeait le site en direct avec un splash natif `launchAutoHide: false` : page jamais chargée → splash jamais fermé → écran figé sans message. Corrigé (build 7) : splash natif fermé seul au bout de 4 s, `server.errorPath: erreur-app.html` (page EMBARQUÉE, `public/`, réessai automatique toutes les 8 s vers https://vit-auto.com) ; test de garde dans `css.test.js`. Décision de l'exploitant : garder le chargement en direct pour la review en cours ; embarquer l'interface dans l'app (assets dans le bundle, API en absolu, CORS `capacitor://vit-auto.com`) en version 1.1 — coût : un build et une revue Apple par changement d'interface. Audit d'affichage complet relancé sur les quatre écrans après le lot mobile : plus aucune commande sous la barre d'état, seules restent des interceptions voulues (dialogue, menu ou messagerie ouverts) et un lien de 37 px sur iPad, corrigé.
