...
// Email ciblé : en argument de ligne de commande d'abord (le cas courant dès
// qu'il existe plusieurs comptes admin), sinon ADMIN_SEED_EMAIL, sinon le
...
async function main() {
  ...
  if (!admin) {
    ...
    const others = await users.find({ role: "admin" }).project({ email: 1 }).toArray();
    if (others.length) {
      console.error(`\n   Comptes admin existants :`);
      others.forEach((a) => console.error(`     • ${a.email}`));
      console.error(`\n   Relancez avec l'un d'eux :`);
      console.error(`     node server/scripts/resetAdminPassword.js <email>`);
    } else {
  ...
  // Un compte qui n'est PAS admin n'est jamais promu au passage : ce script
  // réinitialise un mot de passe, il ne distribue pas de droits. Et si le
  // compte admin n'a encore aucune permission, il reçoit l'accès général
  // (sinon il se retrouverait administrateur sans le moindre droit — voir
  // constants/adminScopes.js).
  const set = { password: hash, isActive: true, emailVerified: true };
  if (admin.role === "admin" && !(admin.adminScope || []).length) {
    set.adminScope = ["super_admin"];
  }
  await users.updateOne({ email: ADMIN_EMAIL }, { $set: set });

  console.log("✅  Mot de passe admin réinitialisé avec succès !\n");
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
  console.log(`   Email        : ${ADMIN_EMAIL}`);
  console.log(`   Mot de passe : ${NEW_PASSWORD}`);
  if (set.adminScope) console.log(`   Permissions  : 👑 administrateur général (aucune n'était attribuée)`);
  console.log("━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━");
