...
import { btn, heroSection, greeting, signature, infoBox, dataTable, divider, stepTimeline } from "../shared/components.js";
...
export function loiSignedTemplate({ firstName, companyName, loiRef, nextStepDate }, trackingPixel = "") {
  const body = `
    ...
    <p style="font-size:14px;color:${BRAND.muted};line-height:1.7;margin:0 0 20px">
      Votre Lettre d'Intention pour <strong>${companyName}</strong> a été signée et enregistrée avec succès.
  ...
  return baseEmail({
    title: "LOI signée",
    preheader: `Votre LOI ${companyName} a été signée — accord partenaire en cours`,
    body,
  });
}
...
export function agreementSignedTemplate({ firstName, companyName, commissionRate, networkRate, dashboardUrl }, trackingPixel = "") {
  const body = `
    ...
    ${greeting(firstName)}
    <p style="font-size:14px;color:${BRAND.muted};line-height:1.7;margin:0 0 20px">
      Félicitations ! L'accord partenaire pour <strong>${companyName}</strong> a été signé et votre compte
  ...
  return baseEmail({
    ...
    preheader: `Bienvenue dans VIT AUTO ! Votre compte ${companyName} est actif`,
    body,
  });
}
