...
import { MESSAGE_ESPECES_PIECE } from "../constants/paiement";
  ...
  return (
    <div className={styles.content} style={{ maxWidth: 720, margin: "32px auto" }}>
      ...
      <div className={dbStyles.driverSummary}>
        {photo
          ? <img src={activePhoto || photo} alt={part.title} className={dbStyles.driverAvatar} style={{ borderRadius: 12, width: 120, height: 120, objectFit: "cover" }} />
      ...
      <div className={dbStyles.fieldBlock}>
        ...
        {needsPosition && <p className={dbStyles.hoursHint} style={{ color: "#b45309" }}>Ce vendeur facture la livraison selon la distance : placez votre adresse sur la carte.</p>}
        {showMap && (
      ...
      <p style={{ margin: "0 0 4px", fontSize: ".86rem", color: "#475569", lineHeight: 1.55 }}>{MESSAGE_ESPECES_PIECE}</p>
      <p style={{ margin: "0 0 18px", fontSize: ".86rem", color: "#475569", lineHeight: 1.55 }}>
        Votre commande est transmise <strong>directement au vendeur</strong>, qui la confirme et vous tient informé(e) de l'expédition. Vous confirmez la réception à la livraison.
      </p>

      <div className={dbStyles.totalBar} style={{ flexDirection: "column", alignItems: "stretch", gap: 6 }}>
        <div style={{ display: "flex", justifyContent: "space-between" }}><span className={dbStyles.totalLabel}>Pièce × {qty}</span><span><PriceTag amountUSD={part.price * qty} pinnedCurrency={part.currency} compact /></span></div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #e2e8f0", paddingTop: 6 }}>
          ...
          <strong className={dbStyles.totalValue}>{quote?.totalUSD == null ? "—" : <PriceTag amountUSD={quote.totalUSD} pinnedCurrency={part.currency} />}</strong>
        </div>
        {quote?.depositUSD > 0 && (
